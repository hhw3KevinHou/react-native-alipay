  ...
  render() {
    ...
    const appRowList = data.map((row, rowIndex) => {
      const appColList = row.map((col, colIndex) => {
        ...
        if (col.isMore) {
          return (
              <View key={'appCol-' + colIndex} style={styles.appColMore}>
                <TouchableHighlight style={{flex: 1,alignItems:'stretch',justifyContent:'center'}}
        ...
        icon = <Image source={col.image.source} style={styles.appItemImageIcon}/>;
           // icon = <Image source={ {uri:'../../../images/iconfont-zhuanzhang.png'}} style={styles.appItemImageIcon}/>;
        } else if (col.icon) {
    ...
    console.log('appRowList'+appRowList);
    return (
